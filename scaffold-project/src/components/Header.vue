<script setup>
</script>

<template>

    <header>
        <section class='grid-area1'>
            <div class="telephone-container">
                <svg class="tel_icon" width="35" height="35" viewBox="0 0 35 35" fill="none" xmlns="http://www.w3.org/2000/svg">
                    <path d="M9.65417 15.7354C11.7542 19.8625 15.1375 23.2312 19.2646 25.3458L22.4729 22.1375C22.8667 21.7438 23.45 21.6125 23.9604 21.7875C25.5938 22.3271 27.3583 22.6188 29.1667 22.6188C29.9688 22.6188 30.625 23.275 30.625 24.0771V29.1667C30.625 29.9688 29.9688 30.625 29.1667 30.625C15.4729 30.625 4.375 19.5271 4.375 5.83333C4.375 5.03125 5.03125 4.375 5.83333 4.375H10.9375C11.7396 4.375 12.3958 5.03125 12.3958 5.83333C12.3958 7.65625 12.6875 9.40625 13.2271 11.0396C13.3875 11.55 13.2708 12.1188 12.8625 12.5271L9.65417 15.7354Z" fill="black"/>
                </svg>
                <!-- PHONE USER -->
                    <input type="text" id="phone" placeholder="Telefone" v-model="userPhone">
            </div>
            <div class="date-container">
                <!-- INPUT DATE  -->
                <input type="date" id="date" v-model="selectedDate">
            </div>

            <!-- FUNCTION TO CALL API -->
            <div class="lupa-container"  @click="searchAPI">    
                <svg class="lupa-icon" width="25" height="25" viewBox="0 0 25 25" fill="none" xmlns="http://www.w3.org/2000/svg">
                    <path d="M20.4167 21.875L13.8542 15.3125C13.3333 15.7292 12.7344 16.059 12.0573 16.3021C11.3802 16.5451 10.6597 16.6667 9.89583 16.6667C8.00347 16.6667 6.40208 16.0111 5.09167 14.7C3.78125 13.3889 3.12569 11.7875 3.125 9.89583C3.125 8.00347 3.78056 6.40208 5.09167 5.09167C6.40278 3.78125 8.00417 3.12569 9.89583 3.125C11.7882 3.125 13.3896 3.78056 14.7 5.09167C16.0104 6.40278 16.666 8.00417 16.6667 9.89583C16.6667 10.6597 16.5451 11.3802 16.3021 12.0573C16.059 12.7344 15.7292 13.3333 15.3125 13.8542L21.875 20.4167L20.4167 21.875ZM9.89583 14.5833C11.1979 14.5833 12.3049 14.1274 13.2167 13.2156C14.1285 12.3038 14.584 11.1972 14.5833 9.89583C14.5833 8.59375 14.1274 7.4868 13.2156 6.575C12.3038 5.66319 11.1972 5.20764 9.89583 5.20833C8.59375 5.20833 7.4868 5.66424 6.575 6.57604C5.66319 7.48785 5.20764 8.59444 5.20833 9.89583C5.20833 11.1979 5.66424 12.3049 6.57604 13.2167C7.48785 14.1285 8.59444 14.584 9.89583 14.5833Z" fill="black"/>
                </svg>
            </div>          
        </section>  
            
        <section class='grid-area2'>
            <svg class="photo-icon" width="45" height="45" viewBox="0 0 45 45" fill="none" xmlns="http://www.w3.org/2000/svg">
                <g id="Person" clip-path="url(#clip0_1_45)">
                <path id="Vector" d="M22.5 22.5C26.6437 22.5 30 19.1438 30 15C30 10.8562 26.6437 7.5 22.5 7.5C18.3562 7.5 15 10.8562 15 15C15 19.1438 18.3562 22.5 22.5 22.5ZM22.5 26.25C17.4937 26.25 7.5 28.7625 7.5 33.75V37.5H37.5V33.75C37.5 28.7625 27.5063 26.25 22.5 26.25Z" fill="black"/>
                </g>
                <defs>
                <clipPath id="clip0_1_45">
                <rect width="45" height="45" fill="white"/>
                </clipPath>
                </defs>
            </svg>               
            <div class="name">
                <spam>João Kleber</spam>
            </div>
        </section>                        
    </header>  


</template>

<script>
  export default {
    methods:{
        searchAPI(){
            // THIS FUNC I`LL CALL API
            console.log('TELEFONE USUARIO', this.userPhone);
            console.log('DATA DESEJADA', this.selectedDate)

        },
    },
    data() {
      return {
        userPhone: '41dkdkdk9',
        selectedDate: '1997-12-03'
      };
    }
  };
</script>

<style scoped>

.grid-area1 {
    display: flex;
    border-radius: 10px;
}

.telephone-container{
    display: flex;
    background-color: white;
    justify-content: center;
    align-items: center;
    width: 380px;
    margin-right: 15px;
    position: relative;
    border-radius: 10px;
    box-shadow: 0 0 2px rgba(0,0,0,.12),0 2px 4px rgba(0,0,0,.24);
}

input {
    border: none;
    text-align: center;
    width: 100%;
    padding: 12px 20px;
    margin: 8px 0;
  }


.tel_icon{
    position: absolute;
    left: 15px;
}

.date-container{
    background-color: white;
    width: 200px;
    margin-right: 15px;
    border-radius: 10px;
    box-shadow: 0 0 2px rgba(0,0,0,.12),0 2px 4px rgba(0,0,0,.24);
}

.lupa-container{
    display: flex;
    justify-content: center;
    align-items: center;    
    background-color: white;
    width: 70px;
    height: 60px;
    border-radius: 10px;
    box-shadow: 0 0 2px rgba(0,0,0,.12),0 2px 4px rgba(0,0,0,.24);
}

.lupa-icon:hover{
    transform: scale(1.2);  
}

.lupa-container:hover{
    cursor: pointer;
}

.grid-area2{
    display: flex;
    align-items: center;
    height: 100px;
}

.photo-icon{
    width: 70px;
    height: 70px;
    margin: 0px 60px; 
    border-radius: 50%;
    background-color: white;
}

.name{
    font-size: 20px;
}

</style>
